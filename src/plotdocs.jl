...
# Usage:
# ```julia
# using PlotReferenceImages, StatsPlots, RDatasets, ProgressMeter, DataFrames, Distributions, StatsBase, Iterators
# generate_doc_images()
# ```
...
const DOC_EXAMPLES = OrderedDict(
    ...
    using Plots, ProgressMeter
        Plots.CURRENT_BACKEND.sym = :pyplot
        Plots.CURRENT_BACKEND.pkg = Plots.PyPlotBackend()
    ...
    "backends_1" => [quote
        using Plots
        Plots.CURRENT_BACKEND.sym = :plotlyjs # Set the backend to PlotlyJS
        Plots.CURRENT_BACKEND.pkg = Plots.PlotlyJSBackend()
        x = 1:10; y = rand(10,2) # 2 columns means two lines
    ...
    "pipeline_1" => [quote
        using Plots
        Plots.CURRENT_BACKEND.sym = :pylot
        Plots.CURRENT_BACKEND.pkg = Plots.PyPlotBackend()
